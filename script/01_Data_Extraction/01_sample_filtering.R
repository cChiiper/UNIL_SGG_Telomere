...
header <- fread("../ukb28603.csv", header = T, nrow = 0)
...
#################################################
## STEP 8: Find samples with self-reported blood malignancies or ICD10 blood malignancy diagnosis
...
# Males only
fwrite(df[which(df$sex == 1), "eid", drop = F], "../lm_samples_M.txt", col.names = F, row.names = F, quote = F, sep = "\t")

# Females only
fwrite(df[which(df$sex == 2), "eid", drop = F], "../lm_samples_F.txt", col.names = F, row.names = F, quote = F, sep = "\t")
