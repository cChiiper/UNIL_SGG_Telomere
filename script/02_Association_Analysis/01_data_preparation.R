...
df <- as.data.frame(fread(file.path(data_folder,  "complete_DF_noeid.txt"), header = T)) 
...
if(sum(df$Zadj_TS != df$TL_add_check) == 0){
  print("No eid issues for added columns")
}

df <- df %>%
  select(-one_of("Zadj_TS.y", "TL_check_array", "TL_add_check", "age.y")) %>%
  rename(blood_cancer = blood_cancer_TF)

### Remove patients below 40 and above 70 years old
df <- filter(df, age >= 40 & age <= 70 )
