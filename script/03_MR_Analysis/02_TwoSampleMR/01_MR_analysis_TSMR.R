...
par_reverse <- FALSE # traits --> st_TRAIT if TRUE
...
HLA_rsids <- fread(file.path(data_folder,  "HLA_rsids.txt"), header = F)
HBB_rsids <- fread(file.path(data_folder,  "HBB_rsids.txt"), header = F)

################################################
### Load regression/already computed MR results 

### File containing FiledID, pheno and added .ma format phenotype names
lm_res <- read.csv(file = file.path(path_to_results,  "TL_metadata.csv"), sep = ',', header = TRUE)
...
for (TRAIT in list_of_traits) {
  ...
  file_name <- paste0(EXPOSURE, "_to_", OUTCOME, "_MR_data.tsv") 
  ...
  if(!identical(file_name, character(0))){
    ...
    dat <- filter(dat, !SNP %in% HBB_rsids$V1)
    ...
    ### Sensitivity
    # dat <- filter(dat, SNP %in% rownames(df_filtered)) #From 07_TL_sensitivity_analysis.R
    # print(paste0("Sensitivity nb SNPs: ", nrow(dat)))
